...
export default class ImageManipulator extends Component {
      ...
      () => {
        ...
        if (isPreResize && preCanvasDimensions) {
          new window.Caman(canvas, function () {
            ...
            this.render(() => {
              const resizedCanvas = that.replaceWithNewCanvas(
  ...
  initializeCanvases = (elem) => {
    ...
    if (isLowQualityPreview && elem.height > 1050) {
      ...
      setTimeout(() => {
        new window.Caman(
          getCanvasNode("scaleflex-image-edit-box"),
          function () {
            this.resize({ width: zoomedWidth, height: zoomedHeight });

            this.render(() => {
              const canvasZoomed = that.replaceWithNewCanvas(
                "scaleflex-image-edit-box"
              );
      ...
      setTimeout(() => {
        that.CamanInstance = new window.Caman(
          ...
          function () {
            updateState({
              isShowSpinner: false,
              canvasOriginal: that.cloneCanvas(
                getCanvasNode("scaleflex-image-edit-box")
              ),
            });
          }
  ...
  makeCanvasSnapshot = (
    ...
    if (previewCanvas) {
      ...
      const zoomedCanvas = this.cloneCanvas(
        getCanvasNode("scaleflex-image-edit-box")
      );
      const nextOperation = {
        ...operation,
    ...
    if (isZoomed) {
      ...
      this.CamanInstanceZoomed.render(() => {
        const canvasZoomed = this.replaceWithNewCanvas(
          "scaleflex-image-edit-box",
          roundCrop
        );
        const nextOperation = {
      ...
      this.CamanInstance.render(() => {
        const canvas = this.replaceWithNewCanvas(
          "scaleflex-image-edit-box",
          roundCrop
        );
        const nextOperation = {
  ...
  getWatermarkArguments = () => {
    ...
    const { x, y, opacity, ...watermark } = shapeOperations.getShape({
      key: WATERMARK_UNIQUE_KEY,
    });
    const {
      original: { width: imgWidth, height: imgHeight } = {},
    } = this.state;
    const { width: canvasWidth, height: canvasHeight } = getCanvasNode(
      PREVIEW_CANVAS_ID
    );
